...
uint8_t *pico_flash_read(long int FLASH_TARGET_OFFSET, size_t len) 
{
    const uint8_t *flash_target_contents = (const uint8_t *) (XIP_BASE + FLASH_TARGET_OFFSET);
    uint8_t *stored_value = calloc(len, sizeof(uint8_t));
...
int pico_flash_write(long int FLASH_TARGET_OFFSET, uint8_t flash_data[], size_t num)
{
    const uint8_t *flash_target_contents = (const uint8_t *) (XIP_BASE + FLASH_TARGET_OFFSET);
    uint8_t *stored_value = (uint8_t *)malloc(num + 1);
